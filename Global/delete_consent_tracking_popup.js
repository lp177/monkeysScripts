...
(function () {
    ...
    function applyStyleAnPreserve(target, style) {
        setTimeout(
            (
                (target, style) => () =>
                    document.querySelector(target).setAttribute("style", style)
            )(target, style),
            10,
        );
        setTimeout(
    ...
    function removePopUp(
        ...
        if (removeClassFromBody) removeCssClass(removeClassFromBody);
        if (setBodyStyle) applyStyleAnPreserve("body", setBodyStyle);
        if (setHtmlStyle) applyStyleAnPreserve("html", setHtmlStyle);
    ...
    function outputDebug(
        msg,
        verbose = true,
        prefix = "popup remover match: ",
    ) {
        if (verbose === false) return;
        console.info(prefix, msg);
    }
    function rearmRemovePopUp(
        popUpSelector,
        removeClassFromBody = null,
        setBodyStyle = null,
        setHtmlStyle = null,
    ...
    function rearmRemoveCssClassUntilSuccess(classNames, max_rearm = 100) {
        if (!removeCssClass(classNames) && --max_rearm > 0)
            setTimeout(
                () => rearmRemoveCssClassUntilSuccess(classNames, max_rearm),
                100,
            );
    }
    ...
    function sheduleOverflowRepare(run_count = 0) {
        ...
        if (
            ...
            body.getAttribute("style").indexOf("overflow: hidden") > -1
        )
            return body.setAttribute(
                "style",
                body.getAttribute("style").replace("overflow: hidden", ""),
            );
        const class_to_remove_from_the_body = [
            "overflowHidden",
            "overflow-hidden",
            "noScroll",
            "onetrust-no-scroll",
        ...
        for (let class_to_remove of class_to_remove_from_the_body) {
            if (body.classList.contains(class_to_remove))
                return body.classList.remove(class_to_remove);
        }
        if (run_count < 30)
    ...
    function launchAllDetection() {
        ...
        if (
            removePopUp(
                "#onetrust-consent-sdk,#cookiebanner,#cookies,#privacy-consent,#CybotCookiebotDialog,#privacy-cookie-banners-root,#dpr-manager,#axeptio_overlay,#gdpr-consent,#js-message-register,#tae-cookie-notice,#consent-manager-container,#usercentrics-root" +
                    ...
                    ",#__consent.md-consent,#onetrust-consent-sdk,#cmpwrapper,#popin-cookie,.eu-cookie-compliance-banner,.js-consent-banner,.cookie-banner,.cookie-banner-layer,.cookie-consent,.cookie-policy,.incentive-banner,.gdpr-settings,.fig-consent-banner,.global-site-notice.notice-cookie,.cookie-card_container" +
            ...
            clickClose(
                '#popin_tc_privacy_button_2,#footer_tc_privacy_button_3,.cookie-banner-content #wt-cli-reject-btn,#vidal_consent a[name="button-refuse"],#gdpr-reject-btn,#cmpbox #cmpbntnotxt,' +
                    '.moove-gdpr-infobar-reject-btn,button[action-type="DENY"][data-tracking-control-name="ga-cookie.consent.deny.v4"],.cookiebanner__buttons__deny,#privacy-cat-modal .btn.refuse-all',
            )
        )
            return outputDebug("Generic clickClose", verbose);
        else if (
            document.querySelector(
                '#qc-cmp2-ui button + button[mode="secondary"]',
            )
        ) {
            document
                .querySelector('#qc-cmp2-ui button + button[mode="secondary"]')
                .click();
            ...
            removePopUp(
                'div[data-portal-id="modal-portal-1"]',
                "scrollDisabled",
            )
        )
            ...
            removePopUp(
                "#public_post_contextual-sign-in,#public_profile_contextual-sign-in",
                "no-scroll",
            )
        )
            return outputDebug(
            ...
            removePopUp(
                'body[style="overflow: hidden;"] div[role="presentation"],body[style="overflow: auto;clear177:true;"] div[role="presentation"]',
                null,
                "overflow: auto;clear177:true;",
            )
        )
            return outputDebug("Generic modal who blocks scrolling", verbose);
        else if (
            removePopUp("#iubenda-cs-banner", null, null, "overflow: auto;")
        )
            return outputDebug("#iubenda-cs-banner", verbose);
        else if (
            removePopUp('div[id^="sp_message_container_"]', "sp-message-open")
        )
        ...
        )
		{
            ...
            removePopUp('div[data-t="cookiesMessage"]', "overflow--hidden")
        ) {
            document.querySelector(".popover-portal")?.remove();
            return outputDebug('div[data-t="cookiesMessage"]', verbose);
        } else if (removePopUp(".privacy-consent--modal")) {
            document.querySelector(".privacy-consent--backdrop")?.remove();
            ...
            document
                .querySelector("html")
                .removeAttribute("data-fast-cmp-locked");
            return outputDebug("#cplus_content_modal", verbose);
        } else if (removePopUp("#gdpr-privacy-settings", "wt-body-no-scroll")) {
            document.querySelector("#gdpr-single-choice-overlay")?.remove();
            return outputDebug("#gdpr-privacy-settings", verbose);
        } else
            document.querySelector("html").className.includes("sd-cmp") &&
                document
                    .querySelector("html")
