...
(function () {
    ...
    function refreshIfCountItemsHighlightableChange() {
        ...
        if (actualCount != highlightableItemsCount) location.reload();
        window.setTimeout(refreshIfCountItemsHighlightableChange, 15000);
    }
    function notifyStatusChange() {
        const status = document.querySelector(
            '[data-testid="active-order-sticky-eta"] div',
        );
        if (!status) return;
        ...
        localStorage.setItem("ubereat_alert_on_change", status.innerText);
    }
    function update() {
        if (document.querySelector("footer")) {
            ...
            document
                .querySelector(
                    "#main-content > div:nth-of-type(2) > div:nth-of-type(2)",
                )
                ?.setAttribute("style", "bottom:0px;top:0px;");
            document.querySelector("header")?.remove();

            document
                .querySelector(
                    "#main-content > div:last-of-type > div:last-of-type",
                )
                ?.setAttribute("style", "top:0!important;bottom:0!important;");
            document
