...
(function () {
    ...
    function createButton(retried = 0) {
        const profileButton = document.querySelector(
                'nav.menu-holder .sub-menu a[href^="https://bsaber.com/members/"]',
            ),
            bookmarksButton = document.querySelector(
                'nav.menu-holder .sub-menu a[href^="https://bsaber.com/songs/new/?bookmarked_by="]',
            );
        if (!profileButton)
            return retryLater(
                retried,
                "Fail to find profil button at: document.querySelector('nav.menu-holder .sub-menu a[href^=\"https://bsaber.com/members/\"]')",
            );
        if (!bookmarksButton)
            return retryLater(
                retried,
                "Fail to find bookmarks button at: document.querySelector('nav.menu-holder .sub-menu a[href^=\"https://bsaber.com/songs/new/?bookmarked_by=\"]')",
            );
        ...
        myBookmarksButton.querySelector("a").href = bookmarksButton.href
            .replace("=members", pseudo)
            .replace("=%user%", pseudo);
        myBookmarksButton.querySelector("a").innerText = "My bookmarks";
        bookmarksButton.parentNode.parentNode.insertBefore(
            myBookmarksButton,
            bookmarksButton.parentNode,
        );
    }
    createButton();
})();
