...
(function() {
    ...
    function search_and_destroy(max_retry = 50)
    {
        ...
        let neighbour = document.querySelector('div[class^="Toasts__Container-"]+div p a[href^="https://www.coinbase.com/legal/cookie"]');
        if (!neighbour)
            return setTimeout(()=>search_and_destroy(max_retry - 1), 300);
        let button = neighbour.parentNode.parentNode.querySelector('div > button:first-of-type');
        if (!button)
            return setTimeout(()=>search_and_destroy(max_retry - 1), 300);
        if (button.innerText==='Close'||button.innerText==='Fermer')
        {
            ...
            console.info("bad text:", button.innerText);
    }
    search_and_destroy();
})();
