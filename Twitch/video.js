...
(function() {
	...
	{
		if(window.location.href.startsWith('https://www.twitch.tv/directory/game/'))return;
		const enter_in_live=document.querySelector('a div[data-a-target="tw-core-button-label-text"]');
		if(enter_in_live)return enter_in_live.click();
		if(max_retry>0)setTimeout(()=>autoClickOnButtonEnterInLive(max_retry-1),300);
	}
	function removeStoriesNavSection()
	{
		const stories_elements=document.querySelectorAll('*[class*="storiesLeftNavSection-"]');
		for (let stories_element of stories_elements)
			stories_element.remove();
	}
	function updateVideos()
	{
		...
		const turboButton=document.querySelector('nav[data-a-target="top-nav-container"] svg path[d="M10.114 9.622 11 7 7.175 9.323a.382.382 0 0 0 .013.65l.698.405L7 13l3.825-2.323a.382.382 0 0 0-.012-.65l-.699-.405z"]');
		if(turboButton)
			turboButton.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.remove();
		...
		document.querySelector('.community-points-summary .claimable-bonus__icon')?.click(); // Click on chest button for collect points
		...
		document.querySelector('.video-player__overlay')?.addEventListener('click',(e)=>{
