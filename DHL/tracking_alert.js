...
(function() {
    ...
    async function alertOnChange() {
		const wrapper=document.querySelector('.c-tracking-result--section span[aria-hidden="true"]');
        if (!wrapper)
            return; // Nothing to track avoid to refresh without track id

        const track_id = wrapper.innerText;
        const last_saved_value = localStorage.getItem('dhl_alert_on_change.' + track_id);
		const actual_state = document.querySelector('.c-tracking-result--status-copy-message').innerText;
        ...
        if (last_saved_value === actual_state)
            return;
