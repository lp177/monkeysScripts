...
(function () {
    ...
    function switchToMostRecent() {
        var select = document.querySelector(
            "#cm-cr-dp-review-sort-type span span select",
        );
        if (isVisible(select)) {
            if (
                document.querySelector(
                    '#cm-cr-sort-dropdown[aria-pressed="false"],#cm-cr-sort-dropdown:not([aria-pressed])',
                )
            )
                select.click();
            if (
                document.querySelector(`a[data-value='{"stringVal":"recent"}']`)
            )
                return document
                    .querySelector(`a[data-value='{"stringVal":"recent"}']`)
                    .click();
        }
